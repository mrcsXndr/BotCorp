...
// getUpdates?timeout=0&limit=1 with NO offset, so nothing queued is confirmed or
// dropped; stops at the first 409. The token only ever goes into the URL.
export async function tgSlotProbe(token, { base = 'https://api.telegram.org', n = 4, gapMs = 2000, timeoutMs = 10_000 } = {}) {
  const codes = [];
  for (let i = 0; i < n; i++) {
    try {
      const r = await fetch(`${base}/bot${token}/getUpdates?timeout=0&limit=1`, { signal: AbortSignal.timeout(timeoutMs) });
      codes.push(r.status);
      try { await r.arrayBuffer(); } catch {}
    } catch { codes.push(0); }
    if (codes[codes.length - 1] === 409) break;
