      ...
      if (cfg.harness.modules.telegram) {
        ...
        if (ownPoller !== 'OWNED') {
          if (flags['no-tg-probe']) skipped = '--no-tg-probe';
            ...
            // the vault runs the probe itself: plaintext leaves it only for an attested launch
            const pr = runPwshFile(SECRETS_PS1, ['-Bot', bot, '-Action', 'tg-probe', '-BotCorpRoot', ROOT], { timeoutMs: 90_000 });
            let j = null; try { j = JSON.parse(pr.out.trim().split(/\r?\n/).pop() || ''); } catch {}
            if (pr.code !== 0 || !j) skipped = `the vault probe failed: ${scrub((pr.err || pr.out).trim()).split(/\r?\n/)[0].slice(0, 120) || `exit ${pr.code}`}`;
            else if (j.skipped) skipped = String(j.skipped);
            else codes = Array.isArray(j.codes) ? j.codes.map(Number) : [];
          }
        }
