...
- doctor `<bot>: foreign telegram owner-lock` (a launcher outside BotCorp) and
  ...
  is not the holder, no offset, stops at the first 409). On this line the
  probe is `secrets.ps1 -Action tg-probe`: the token never leaves the vault.
"""
...
def pushable(tmp_path):
    ...
    home = tmp_path / "bots" / "pushbot"      # the hook runs only in a launcher-set bots/<BOT_NAME> home
...
def _tg_probe(name: str, env: dict, base: str) -> subprocess.CompletedProcess:
    return subprocess.run(["pwsh", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", str(ASSEMBLY / "daemon" / "secrets.ps1"),
                           "-Bot", name, "-Action", "tg-probe", "-TgApiBase", base], capture_output=True, text=True, timeout=120, cwd=str(ASSEMBLY), env=env)


@needs_pwsh
@pytest.mark.parametrize("codes,want", [([200, 409, 200], [200, 409]), ([200, 200, 200, 200, 200], [200, 200, 200, 200])])
def test_the_vault_runs_the_slot_probe_and_prints_codes_only(repo_bot, codes, want):
    name, home, rt, env = repo_bot
    (home / "bot.yaml").write_text(f"name: {name}\n", encoding="utf-8")
    value = "not-a-real-value"
    r = subprocess.run(["pwsh", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-Command",
                        f". '{ASSEMBLY / 'daemon' / 'vault.ps1'}'; [void](Set-VaultSecret -BotHome '{home}' -Bot '{name}' -Key telegram_token -Value '{value}')"],
    ...
    try:
        r = _tg_probe(name, env, f"http://127.0.0.1:{srv.server_address[1]}")
    finally:
        srv.shutdown()
    assert r.returncode == 0, r.stderr
    assert value not in r.stdout + r.stderr
    assert json.loads(r.stdout.strip().splitlines()[-1]) == {"codes": want}
    assert _Tg.paths and all(p == f"/bot{value}/getUpdates?timeout=0&limit=1" for p in _Tg.paths), _Tg.paths   # no offset, ever


@needs_pwsh
def test_the_vault_probe_skips_without_a_token_and_refuses_a_foreign_base(repo_bot):
    name, home, rt, env = repo_bot
    (home / "bot.yaml").write_text(f"name: {name}\n", encoding="utf-8")
    r = _tg_probe(name, env, "http://127.0.0.1:9")
    assert r.returncode == 0, r.stderr
    assert json.loads(r.stdout.strip().splitlines()[-1]) == {"skipped": "no telegram_token in the vault"}
    r = _tg_probe(name, env, "https://example.invalid")
    assert r.returncode != 0 and "must be http://127.0.0.1" in r.stderr
