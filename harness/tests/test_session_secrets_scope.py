...
'seeded'
"nonce=$(New-LaunchNonce -Bot '{name}')" """, env=env)
        assert seed.returncode == 0 and "seeded" in seed.stdout, seed.stderr
        # an attested launch (this line's trusted start paths mint the nonce); without one nothing is decrypted
        env["BOTCORP_LAUNCH_NONCE"] = next(ln for ln in seed.stdout.splitlines() if ln.startswith("nonce="))[len("nonce="):].strip()
