...
//   bots/<name>/tools/tg/<tool>.py                    forwarding SHIM per harness tools/tg/*.py
//                                                     (toolShimText): the rules and CLAUDE.md say
//                                                     `python tools/tg/tg_send.py`, relative to the
//                                                     bot folder, but the tool ships in the harness.
...
// ---- tools/tg shims ------------------------------------------------------------
// The harness rules, the bot template's CLAUDE.md and the reply nudge all say
// `python tools/tg/tg_send.py ...`, run from the bot folder; the tool itself
// lives at <BotCorp>/harness/tools/tg/. The shim runs the harness copy in the
// same interpreter (argv, stdin, exit code pass through), so there is one copy
// of the code and a harness update needs no re-sync. It finds the harness
// relative to itself (bots/<bot>/tools/tg -> <BotCorp>), then at the checkout
// sync baked in (a bot folder reached through a junction resolves elsewhere);
// never a plugin-cache version path.
export const SHIM_MARKER = '# botcorp-shim: generated by botcorp sync';
export const SHIM_DIRS = ['tg'];

export function toolShimText(rel, botcorpRoot) {
  return [
    SHIM_MARKER,
    '# Runs the harness copy of this tool. Delete the first line to make the file',
    '# this bot\'s own; sync then never touches it.',
    'import runpy',
    'import sys',
    'from pathlib import Path',
    '',
    `REL = ${JSON.stringify(rel)}`,
    `BOTCORP = ${JSON.stringify(fwd(path.resolve(botcorpRoot)))}`,
    '',
...
function readHead(file) {
  try { return fs.readFileSync(file, 'utf-8').split(/\r?\n/, 1)[0]; } catch { return null; }
}
...
export function toolShimState(botHome, botcorpRoot) {
  ...
  for (const dir of SHIM_DIRS) {
    const src = path.join(botcorpRoot, 'harness', 'tools', dir);
    let names = [];
    try { names = fs.readdirSync(src, { withFileTypes: true }).filter(d => d.isFile() && d.name.endsWith('.py')).map(d => d.name).sort(); } catch {}
    const destDir = path.join(botHome, 'tools', dir);
    for (const n of names) {
      const head = readHead(path.join(destDir, n));
      rows.push({ rel: `tools/${dir}/${n}`, kind: head === null ? 'missing' : head === SHIM_MARKER ? 'shim' : 'own' });
    }
    let present = [];
    try { present = fs.readdirSync(destDir).filter(n => n.endsWith('.py') && !names.includes(n)); } catch {}
    for (const n of present.sort()) if (readHead(path.join(destDir, n)) === SHIM_MARKER) rows.push({ rel: `tools/${dir}/${n}`, kind: 'stale' });
  }
  return rows;
}
...
function syncToolShims(botHome, botcorpRoot, dry, report) {
  for (const row of toolShimState(botHome, botcorpRoot)) {
    ...
    report[row.rel] = writeIfChanged(dest, toolShimText(row.rel, botcorpRoot), dry);
  }
}
