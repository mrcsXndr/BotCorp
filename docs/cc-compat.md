...
| iii | The Telegram plugin takes `TELEGRAM_BOT_TOKEN` from the process env with no `channels/telegram/.env` | **confirmed 2026-09-25 2.1.282** with a dummy token (`123456789:AA...`, never a real one) in a scratch bot: env only, no `.env`, the plugin got past its token check and wrote `bot.pid` (bun `server.ts` under the session's claude). **Under `--bg` only when the config home's daemon was started by that same launch**: a session spawned by an already-running daemon gets the DAEMON's env (its MCP log: `telegram channel: TELEGRAM_BOT_TOKEN required`, no `bot.pid`) | `launch.ps1 -Bg` stops a daemon that has no live session before `claude --bg` (docs/daemon.md, "bg sessions and the daemon's env"); `harness.telegram_token_file: true` stays as a transient fallback, deleted once the plugin read it |
