...
The bot's rules and `CLAUDE.md` run the Telegram tools relative to the bot
folder (`python tools/tg/tg_send.py ...`), but they ship in the harness. So
sync writes a forwarding shim `bots/<name>/tools/tg/<tool>.py` for every
`harness/tools/tg/*.py`: it runs the harness copy in the same interpreter
(argv, stdin and exit code pass through), found relative to the shim
(`bots/<name>` -> the BotCorp checkout) or else at the checkout sync ran
from, never a plugin-cache version path, so a harness update needs no
re-sync. A shim's first line is `# botcorp-shim: generated by botcorp sync`.
A file without that line is the bot's own and is never touched (delete the
line to take a shim over); a shim whose harness tool is gone is removed.
